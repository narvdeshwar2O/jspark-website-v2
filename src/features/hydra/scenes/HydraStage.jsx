...
export default function HydraStage({ registerEl, onCaptureReady }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Sky: dark background (no stars) but keep realistic atmosphere on the globe
      ...
      scene.globe.baseColor = Cesium.Color.BLUE

      // Lighting: Realistic sun lighting. Set time to mid-day over India for good shadows
      viewer.clock.currentTime = Cesium.JulianDate.fromIso8601('2024-05-15T06:00:00Z')
      scene.light = new Cesium.SunLight()
      scene.globe.enableLighting = true
      // Use natural daylight lighting settings for realistic terrain depth
      scene.globe.lambertDiffuseMultiplier = 1.2
      scene.globe.vertexShadowDarkness = 0.4
      ...
      try {
        ...
        // Use real color, default values
        terrainImagery.brightness = 1.0
        terrainImagery.contrast = 1.0
        terrainImagery.saturation = 1.0
      } catch (err) {
      ...
      // Unblock prefly immediately so hero and initial globe load in <1s
      ...
      // Resolve scene 3D objects in background
